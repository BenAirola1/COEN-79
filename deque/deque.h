...
namespace coen79_lab8
{
    ...
    public:
        ...
        deque(size_type init_bp_array_size = BLOCKPOINTER_ARRAY_SIZE, size_type init_block_size = BLOCK_SIZE);
        ...
        for (size_type bp_array_index = 0; bp_array_index < source.bp_array_size; ++bp_array_index)
        {
            ...
            else
            {
                ...
                for (size_type block_item_index = 0; block_item_index < block_size; ++block_item_index)
                {
                    ...
                    // Copies element to current array MIGHT NOT WORK
    ...
    void deque<Item>::push_front(const value_type& entry)
    {
        ...
        if (first_bp == NULL)
        {
            assert(bp_array_size > 1); // why not >0 ?
    ...
    void deque<Item>::pop_back()
    {
        ...
        // This is the only entry in the deque
        if (back_ptr == front_ptr)
        {
            clear();
        }
        // This is the last entry of the last data block
    ...
    // Constructs an iterator which points to the
    ...
    // Constructs an iterator which points past the
